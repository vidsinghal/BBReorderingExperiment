; ModuleID = '/local/scratch/a/singhav/BBReorderingExperiment/INTERESTING_FILES_LLC_DISABLE_BLOCK_PLACEMENT//504/reordered.bc'
...
vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  ...
  %0 = icmp eq i64 %index.next13, %n.vec6
...
for.body4:                                        ; preds = %for.body4.preheader, %for.body4
  ...
  %exitcond.not = icmp eq i64 %1, 0
  br i1 %exitcond.not, label %for.end, label %for.body4, !llvm.loop !3
...
for.body4.preheader:                              ; preds = %iter.check, %vec.epilog.iter.check, %vec.epilog.middle.block
  %indvars.iv.ph = phi i64 [ %4, %iter.check ], [ %ind.end7, %vec.epilog.iter.check ], [ %ind.end, %vec.epilog.middle.block ]
  br label %for.body4
...
vector.ph:                                        ; preds = %vector.main.loop.iter.check
  ...
  %invariant.gep = getelementptr i8, ptr %_dest, i64 %4
  br label %vector.body

vec.epilog.middle.block:                          ; preds = %vec.epilog.vector.body
  %cmp.n8 = icmp eq i64 %n.vec6, %5
  br i1 %cmp.n8, label %for.end, label %for.body4.preheader

vec.epilog.ph:                                    ; preds = %vector.main.loop.iter.check, %vec.epilog.iter.check
  %vec.epilog.resume.val = phi i64 [ %n.vec, %vec.epilog.iter.check ], [ 0, %vector.main.loop.iter.check ]
  %n.vec6 = and i64 %5, 4294967288
  %ind.end = add nsw i64 %n.vec6, %4
  ...
  %invariant.gep14 = getelementptr i8, ptr %_dest, i64 %4
  br label %vec.epilog.vector.body

iter.check:                                       ; preds = %entry
  %4 = sext i32 %type_size to i64
