; ModuleID = '/local/scratch/a/singhav/BBReorderingExperiment/INTERESTING_FILES_LLC_DISABLE_BLOCK_PLACEMENT//254/reordered.bc'
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %2 = load float, ptr %n, align 4, !alias.scope !0
  %broadcast.splatinsert8 = insertelement <2 x float> poison, float %2, i64 0
  %broadcast.splat9 = shufflevector <2 x float> %broadcast.splatinsert8, <2 x float> poison, <2 x i32> zeroinitializer
  %3 = getelementptr %struct.complex, ptr %ap, i64 %index
  %4 = getelementptr %struct.complex, ptr %ap, i64 %1
  %interleaved.vec = shufflevector <2 x float> zeroinitializer, <2 x float> %broadcast.splat9, <4 x i32> <i32 0, i32 2, i32 1, i32 3>
  store <4 x float> %interleaved.vec, ptr %3, align 4
  store <4 x float> %interleaved.vec, ptr %4, align 4
...
vector.memcheck:                                  ; preds = %for.body19.preheader
  ...
  %scevgep = getelementptr i8, ptr %ap, i64 %7
